...
public abstract class SidecarInputBuilderImpl<T extends ProcessorEvent> implements SidecarInputBuilder<T> {
    ...
    private Set<SidecarInputExpander<T>> expanders = new HashSet<>();
    private Set<CommandExpander<T>> commandExpanders = new HashSet<>();
    ...
    private InspectionResult inspectApplicationEAV(T event) {
        ...
        if (key != null) {
            ...
            if (app != null) {
                return inspectExtendedAttributes(app.getExtendedAttributes(), this.requiredEAVs);
            }
        }
    ...
    private InspectionResult inspectPackageKeyEAVs(T event) {
        ...
        Key key = event.getKey();
        if (key != null) {
            return inspectExtendedAttributes(key.getExtendedAttributes(), this.requiredPackageKeyEAVs);
        }
    ...
    private InspectionResult inspectExtendedAttributes(ExtendedAttributes ea, Set<String> requiredSet) {
        if (ea != null) {
            try {
    ...
    void expandRemoteAddress(ProcessorEvent ppe, SidecarInput input) {
        input.setRemoteAddr(ppe.getCallContext().getRequest().getHTTPRequest().getRemoteAddr());
    }
