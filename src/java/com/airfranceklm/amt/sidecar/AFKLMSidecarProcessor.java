...
public class AFKLMSidecarProcessor implements TrafficEventListener {
    ...
    private static final String PRE_CONDITION_FAILURE_MSG = "Request pre-condition not met, code 0x000003BB";
    private static final String MSG_SERVICE_CANNOT_BE_PROVIDED = "Service cannot be provided, code 0x000003BB";

    private static final String MSG_PREPROCESSING_ERROR = "Internal server error before processing the call, code 0x000003BB";
    private static final String MSG_POSTPROCESSING_ERROR = "Internal server error before sending the response, code 0x000003BB";
    private static final String MSG_PREFLIGHT_ERROR = "Internal server error before processing the call, code 0x000003BB, reason 0x000003B1";
    private static final String MSG_GLOB_ERROR = "Internal server error before processing the call, code 0x000003BB, reason 0x000003B3";
    ...
    public AFKLMSidecarProcessor() {
        ...
        // TODO:
        // - Scan the local files and accept the configuration, if exists;
        // - scan load in-memory stack
    ...
    private SidecarOutput invokeRelayAware(SidecarInvocationData cmd) throws IOException {
        ...
        if (retVal != null && retVal.relaysMessageToPostprocessor()) {
            if (retVal.getRelayMessage() != null) {
                cmd.logEntry(RELAY_MESSAGE, retVal.getRelayMessage());
            }
    ...
    private void sendError(TrafficManagerResponse response, SidecarInputPoint point) {
        ...
        response.getHTTPResponse().setBody(new ContentProducerImpl(String.format("<h1><![CDATA[%s]]></h1>", msg)));
    ...
    private void sendCurtailed(TrafficManagerResponse response, int code, String message) {
        ...
        String content = String.format("<h1><![CDATA[%s]]></h1>", uMessage);
        http.setBody(new ContentProducerImpl(content));
