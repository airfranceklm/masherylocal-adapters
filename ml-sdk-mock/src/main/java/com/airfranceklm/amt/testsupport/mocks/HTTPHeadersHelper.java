...
import static com.airfranceklm.amt.testsupport.Mocks.eqCaseInsensitive;
...
public class HTTPHeadersHelper {
    ...
    public HTTPHeadersHelper(EasyMockSupport owner, Map<String, String> pStartingSet) {
        this.owner = owner;
        this.sourceData = new HashMap<>();
        if (pStartingSet != null) {
            this.sourceData.putAll(pStartingSet);
        }
        this.delegate = new MutableHTTPHeadersImpl(this.sourceData);
    }

    public HTTPHeaders createReadonlyHeaders() {
        ...
        if (sourceData != null) {
            sourceData.forEach((key, value) -> {
                expect(mockHeaders.get(eqCaseInsensitive(key))).andReturn(getStringValueOf(value)).anyTimes();
                expect(mockHeaders.contains(eqCaseInsensitive(key))).andReturn(true).anyTimes();
            });
        }
    ...
    void initAccumulatingMock() {
       ...
       expect(mutableMock.contains(anyString())).andDelegateTo(delegate).anyTimes();
     ...
     * @return
     */
    public MutableHTTPHeaders createMutableHeadersMock() {
